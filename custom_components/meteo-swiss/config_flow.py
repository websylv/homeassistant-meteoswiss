...
class MeteoSwissFlowHandler(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def validate_config(self, config):
        ...
        stationNameChk = await self.hass.async_add_executor_job(
            self._client.get_station_name, config[CONF_STATION]
        )
        if stationNameChk is None:
            self._errors[CONF_STATION] = "invalid_station_id"
            _LOGGER.warning(
                "%s not found in meteo swiss station list" % (config[CONF_STATION])
            )
        ...
        _LOGGER.error("Configuration error for meteo swiss integration")
        return False
    ...
    async def async_step_user(self, user_input=None):
        ...
        self._postCode = int(
            await self.hass.async_add_executor_job(self._client.getPostCode, lat, lon)
        )
        ...
        _LOGGER.debug(
            "Lon : %s - Lat : %s - PostCode %s Station %s Name: %s"
            % (lon, lat, self._postCode, self._station, self._stationName)
        )
